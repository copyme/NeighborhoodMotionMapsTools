...
public:
    ...
    inline
    RealPoint operator()( const RealPoint& aInput ) const
    {
        RealPoint p;
...
template <typename TSpace>
class BijectiveRigidTransformation2D : std::unary_function <typename TSpace::Point, typename TSpace::Point>
{
    // ----------------------- Types ------------------------------
public:
    typedef typename TSpace::Point Point;
...
public:
    ...
    {
        RealPoint realPoint;
        realPoint[0] =  ( ( t_cos * ( aInput[0] - origin[0] ) -
               t_sin * ( aInput[1] - origin[1] ) ) + translation[0] ) + origin[0];

        realPoint[1] = ( ( t_sin * ( aInput[0] - origin[0] ) +
               t_cos * ( aInput[1] - origin[1] ) ) + translation[1] ) + origin[1];

        Point intPoint ( std::round ( realPoint[0] ), std::round ( realPoint[1] ) );

        RealPoint remainder ( realPoint[0] - intPoint[0], realPoint[1] - intPoint[1] );

        if ( remainder[0] > l3 && ( remainder[1] < l3 && remainder[1] > l2 ) )
            intPoint[0] = std::round ( intPoint[0] + remainder[0] *2 );

        else if ( remainder[0] < l0 && ( remainder[1] < l1 && remainder[1] > l0 ) )
            intPoint[0] = std::round ( intPoint[0] + remainder[0] * 2 );

        if ( remainder[1] > l3 && ( remainder[0] > l0 && remainder[0] < l1 ) )
            intPoint[1] = std::round ( intPoint[1] + remainder[1] * 2 );

        else if ( remainder[1] < l0 && ( remainder[0] > l2 && remainder[0] < l3 ) )
            intPoint[1] = std::round ( intPoint[1] + remainder[1] * 2 );
